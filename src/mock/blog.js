...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function (options) {
  ...
  return Mock.mock({
    ...
    data: {
      ...
      [`rows|${query.limit || 10}`]: [
        //rows表示文章列表里面有多少块文章区域，limit是传递进来的参数表示单页容量（即一页放多少个文章块）
        //当前页列表数据
        {
          id: "@guid", //gu-id，表示生成全局唯一的字符串id，用于绑定key值
          title: "@ctitle(1,50)", //文章标题
          description: "@cparagraph(1,10)", //生成1-10个句子的中文段落
          category: {
            //所属分类
            "id|1-10": 0,
            name: "分类@id",
...
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", {
  ...
  data: {
    ...
    description: "@cparagraph(1, 10)", //博客描述，显示到列表页
...
Mock.mock("/api/comment", "post", (options) => {
  return Mock.mock({
    ...
    data: {
      id: "@guid",
      nickname: JSON.parse(options.body).nickname, //昵称
      content: JSON.parse(options.body).content, //评论内容，纯文本
      createDate: Date.now(),
      "avatar|1": [
        "https://img0.baidu.com/it/u=1292432951,908021080&fm=26&fmt=auto",
        "https://img1.baidu.com/it/u=3615882974,1863626711&fm=26&fmt=auto",
        "https://img0.baidu.com/it/u=490313707,3237279572&fm=26&fmt=auto",
        "https://img1.baidu.com/it/u=3179149615,2710619395&fm=26&fmt=auto",
      ], //随机的头像地址
    },
  });
});
    ...
    data: {
      ...
      [`rows|${query.limit || 10}`]: [
        //当前页列表数据
        {
          id: "@guid",
          nickname: "@cname", //昵称
          content: "@cparagraph(1,10)", //评论内容，纯文本
          createDate: Date.now(),
          "avatar|1": [
            "https://img0.baidu.com/it/u=1292432951,908021080&fm=26&fmt=auto",
            "https://img1.baidu.com/it/u=3615882974,1863626711&fm=26&fmt=auto",
